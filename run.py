...
def run_test(model,validation_set,test_name,features,labels,recording_ids,model_arcitechture,segment_parameters, multiclass, loso):
    ...
    if model.upper() == "LDA":
        if validation_set:
            ...
            if loso:
                LDA.loso_cv(features, labels, recording_ids,test_name,segment_parameters)
            else: 
    ...
    # HMM
        ...
        if validation_set:
            ...
            if loso:
                hmm.loso_cv(features, labels, recording_ids, test_name, model_arcitechture, segment_parameters)
            else: 
