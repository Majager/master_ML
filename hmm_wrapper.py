...
class HMM(ClassifierMixin, BaseEstimator):
    ...
    def _predict_segment_proba(self, segments_data, n_features):
    ...
    for idx, recording_data in enumerate(data):
        ...
        for idx_segment, segment in enumerate(labels[idx]):
            if (segment == 1 and meal_found == False):
                ...
                break
        if meal_found == True:
            meal_lengths.extend(idx_meal_end-idx_meal_start)
            nonmeal_lengths.extend(idx_meal_start)
            nonmeal_lengths.extend(len(segment)-idx_meal_end)
        else:
            nonmeal_lengths.extend(recording_data)
